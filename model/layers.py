...
from Attentions import TwoStageAttentionLayer, SingleStageAttentionLayer
...
class SegMerging(nn.Module):
    ...
    def forward(self, x):
        # print('x_seg:',x.shape)
        batch_size, ts_d, seg_num, d_model = x.shape   # x -> [182,dim=55,7,128]
        ...
        return x   # x.shape->[128,55,4(2),128]
...
class scale_block(nn.Module):
    def __init__(self, configs, win_size, d_model, n_heads, d_ff, depth, dropout, \
        ...
        for i in range(depth):
            self.encode_layers.append(SingleStageAttentionLayer(configs, seg_num, factor, d_model, n_heads, \
                                                             d_ff, dropout))
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return encode_x, None     #  encode_x.shape->[128, 55(MSL), 9, 128]
